...
export function NotificationBell() {
  ...
  const urgentCount = alerts.filter((a) => HIGH_URGENCY.has(a.kind)).length;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-84 bg-white rounded-xl shadow-xl border z-50" style={{ width: "22rem" }}>
          <div className="flex items-center justify-between px-4 py-3 border-b">
            ...
            <div className="flex items-center gap-2">
              ...
              {count > 0 && <span className="text-xs text-gray-400">{count} total</span>}
            </div>
          </div>
          ...
          <div className="max-h-96 overflow-y-auto divide-y">
            {alerts.length === 0 ? (
              ...
              alerts.map((alert) => {
                ...
                return (
                  <div key={alert.id} className={`px-4 py-3 hover:bg-gray-50 transition-colors ${cfg.color}`}>
