...
| El filtro «Sin leer» se comprueba con el marcado automatico AISLADO | ✅ | `mensajes.spec.ts` — **prueba inestable corregida**: la preselección automática marca leída la primera conversación, que deja entonces de cumplir «Sin leer» y **desaparece de la lista** a los pocos milisegundos. Tanto la cifra como la pertenencia a la lista eran una carrera contra ese marcado. Ahora el `PUT` de marcar leído se intercepta **sólo en esta prueba**: el listado, su petición y su dibujado siguen siendo los reales, y lo que se comprueba es el filtro, que es lo que promete el nombre. El marcado automático tiene sus propias pruebas |
